...
public class Base64Utils {
    ...
    public boolean base64ToFile(String strBase64,File outFile){
        try {
        	System.out.println("base64转换文件");
            // 解码，然后将字节转换为文件
            byte[] bytes = Base64.getDecoder().decode(strBase64); // 将字符串转换为byte数组
            System.out.println("base64转成byte");
     ...
     * @throws IOException 
     */
    private boolean copyByte2File(byte [] bytes,File file) throws IOException{
        FileOutputStream  out = null;
        try {
        	System.out.println("转换为输入流..");
            //转化为输入流
